...
export class AutocompleteArrayInput extends React.Component {
    ...
    getSuggestionText = suggestion => {
        ...
        //console.log("Label: ", suggestionLabel, suggestion);
    ...
    handleAdd = chip => {
        ...
        // We only go here if the new value is not already in the suggestions-array.
        // dispatch a create-new-category action
        // wait for it to finish
        // on success, change value of chip to the id of the newly created suggestion.
        if (typeof dataProvider === 'function') {
            // TODO: optionText can be a function!
            dataProvider(CREATE, reference, {
                data: {
                    [optionText]: chip
                },
            }).then((json) => {
                input.onChange([...this.state.inputValue, json.data[optionValue]])
            });
            return;
        }
